    ...
    public void  set_min_max(double dmin,double dmax)
{
    ...
    public void init()
    {
        ...
        if (b.width<b.height) scale=b.width/(y_max-y_min);
        else scale=b.height/(y_max-y_min);

	System.out.println(" init- SCALE set to:"+scale + "Height" + height);
...
private void MYdrawLine(Graphics g,float x0,float y0,float x1,float y1)
{
  g.drawLine((int)((x0-y_min)*scale+0.5),(int)(height - (y0-y_min)*scale+0.5),
	     (int)((x1-y_min)*scale+0.5),(int)(height - (y1-y_min)*scale+0.5));
}
...
/* ERH: expecting x_dim as # point in the string y_dim as 2 */
	...
	//System.err.println("partial- SCALE set to:"+scale);
  for(int i=1; i<x_dimension; i++)
     {
       MYdrawLine(g,data[i-1][0][draw_time],
       data[i-1][1][draw_time],
       data[i][0][draw_time],
       data[i][1][draw_time]);
	/*System.err.println("DATA["+(i-1)+"] = "+data[i-1][0][draw_time]
			  +" , "+data[i-1][0][draw_time]);	
	*/
     }
  g.setColor(Color.black);
  g.drawString("Layer: "+canvas_name,5,12);
