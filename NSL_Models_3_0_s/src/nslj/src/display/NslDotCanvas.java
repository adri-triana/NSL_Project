...
public class NslDotCanvas extends NslCanvas
{
 ...
 private Vector2 priv=new  Vector2();
    ...
    public NslDotCanvas(NslFrame nslDisplayFrame, String full_name, 
    ...
    public void  set_min_max(double dmin,double dmax)
{
	...
	Rectangle b = getBounds();
        ...
        g.drawRect(1, 1, b.width-4, b.height-4);
        if (b.width<b.height) scale=(b.width-4)/(y_max-y_min);
        else scale=(b.height-4)/(y_max-y_min);
        height=b.height;
...
private void MYdrawPoint(Graphics g,float x0,float y0)
{
 g.drawOval((int)((x0-y_min)*scale+0.5),(int)((height-(y0-y_min)*scale)+0.5)
	    ,3, 3);
}
	...
	if ((NslSystem.init_run_char == 'B')||
	   ...
	   //System.out.println("debug: NslDotCanvas bad flag "+ NslSystem.init_run_char);
...
/* ERH: expecting x_dim as # point in the string y_dim as 2 */
	...
	if (b.width<b.height) scale=(b.width-4)/(y_max-y_min);
        ...
        height=b.height;
	//System.err.println("SCALE set to:"+scale);

  for(int i=0; i<x_dimension; i++)
     {
       MYdrawPoint(g,data[i][0][draw_time], data[i][1][draw_time]);
       //System.err.println("points:"+data[i][0][draw_time]+","+data[i][1][draw_time]);
     }
  //g.setPaintMode();
  //super.paint(g, 1);       
g.setColor(Color.black);
g.drawString("Layer: "+super.canvas_name,5,12);
...
void FitCurve(Vector2 []d, int nPts, double error, PrintStream ps)
  /*     *d;          Array of digitized points   */
...
//printf("\n\nThere are %d nPts.\n", nPts);
 ...
 //System.out.println("tHat1 "+tHat1.xval()+" "+tHat1.yval()+" tHat2 "+tHat2.xval()+" "+tHat2.yval() );
    ...
    FitCubic(d, 0, nPts - 1, tHat1, tHat2, error, ps);
...
public void FitCubic(Vector2 [] d, int first, int last, 
		     Vector2 tHat1, Vector2 tHat2, double error, PrintStream ps)
...
//	System.out.println("B1 "+B1(uPrime[i])+" B2 "+B2(uPrime[i]));
    ...
    for (i = 0; i < nPts; i++) {
        C[0][0] += V2Points(A[i][0], A[i][0]);
                C[0][1] += V2Points(A[i][0], A[i][1]);
/*                                      C[1][0] += V2Points(A[i][0], A[i][1]);*/
                C[1][0] = C[0][1];
                C[1][1] += V2Points(A[i][1], A[i][1]);
        ...
        X[0] += V2Points(A[i][0], tmp);
        X[1] += V2Points(A[i][1], tmp);
    }
...
/* return the dot product of vectors a and b */
  public double V2Points(Vector2 a, Vector2 b)
  {
...
public void DrawBezierCurve(int n, Vector2 [] curve, PrintStream ps)
{
  int i;
  ...
  for (i=0; i<n; i++){
   ...
   //ps.println(curve[i].xval()+" "+curve[i].yval() );
  }
  // System.out.println("\n\n\n\n\n\n\n");
}
...
/*
 System.out.print("Fitting curves at time "+drawing_time+"\n\n\n");
    ...
    FitCurve(d, pts, error, ps);	*/	 /* Fit the Bezier curves */
