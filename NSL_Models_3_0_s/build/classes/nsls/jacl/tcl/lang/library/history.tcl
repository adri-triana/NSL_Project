...
#
# RCS: @(#) $Id: history.tcl,v 1.1.1.1 1998/10/14 21:09:21 cvsadmin Exp $
#
...
#	A formatted history list
 ...
 proc tcl::HistInfo {{num {}}} {
	...
	# The regsub command is not yet implemented.  So, replace
	# this regsub call with a very simple temporary solution:

	set tmpCmd $cmd
	set cmd ""
	set tmpIndex [string first "\n" $tmpCmd]
	while {$tmpIndex != -1} {

	    append cmd "[string range $tmpCmd 0 $tmpIndex]\t"
	    set tmpCmd [string range $tmpCmd [expr $tmpIndex + 1] end]
	    set tmpIndex [string first "\n" $tmpCmd]
	}
	append cmd $tmpCmd

	# end of temporary replacement.
	# original regsub call follows:
