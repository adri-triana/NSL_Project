	...
	implements MouseMotionListener, MouseListener, KeyListener
{
  ...
  int grid=8;
    ...
    */
    ...
    WarningDialogOkCancel warningPopupOkCancel=null;
    //-------------------------------------------------------
    /**
	...
	for (ix=schDrawables.size()-1; ix>=0; ix--) {
		...
		if (iconInst.selectport(x,y)) {
			...
			if (parentFrame.status.equals("Connection")) {
			    ...
			    parentFrame.status="";
			    currModule.mySchematic.selComponent=(Component)conn;
			    found=schDrawables.size()-1;
			    break schDrawablesLoop;
	    ...
	    if (tempComponent instanceof Connection &&  (parentFrame.status.equals("Connection") == false  ) ) {
  ...
  public void mouseReleased(MouseEvent e) {
	...
	Connection conn=(Connection)sip.links.elementAt(i);
	conn.mergesrc();
      }
