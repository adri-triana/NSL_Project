...
public class  SCSUtility {
 ...
 //   public	static String user_home=System.getProperty("user.home");
    public	static String user_home=System.getProperty(".");
	...
	if ( colstring.equals("BLUE") == true )
	    resultCol = Color.blue ;
	else if
	    ( colstring.equals("RED") == true	   )
	    resultCol = Color.red ;
	else if
	    ( colstring.equals("GRAY") == true	   )
	    resultCol = Color.gray ;
	else if
	    ( colstring.equals("LIGHTGRAY") == true	   )
	    resultCol = Color.lightGray ;
	else if
	    ( colstring.equals("DARKGRAY") == true	   )
	    resultCol = Color.darkGray ;
	else if
	    ( colstring.equals("YELLOW") == true	   )
	    resultCol = Color.yellow ;
	else if
	    ( colstring.equals("GREEN") == true	   )
	    resultCol = Color.green ;
	else if
	    ( colstring.equals("BLACK") == true	   )
	    resultCol = Color.black ;
	else if
	    ( colstring.equals("CYAN") == true	   )
	    resultCol = Color.cyan ;
	else if
	    ( colstring.equals("MAGENTA") == true	   )
	    resultCol = Color.magenta ;
	else if
	    ( colstring.equals("ORANGE") == true	   )
	    resultCol = Color.orange ;
	else if
	    ( colstring.equals("PINK") == true	   )
	    resultCol = Color.pink ;
	else if
	    ( colstring.equals("WHITE") == true	   )
...
public   static  String   returnColorNameString ( Color p_color ) {
  ...
  if ( p_color.equals(    tempcol   = Color.blue ) )
    result = "BLUE";
  
 else if ( p_color.equals(    tempcol   = Color.white ) )
    result = "WHITE";
  
  else if ( p_color.equals(   tempcol =  Color.red ) )
    result = "RED";

  else if ( p_color.equals(     tempcol =  Color.gray ) ) 
    result   =  "GRAY";

  else if ( p_color.equals(     tempcol =  Color.lightGray ) ) 
    result   =  "LIGHTGRAY";
  
  else if ( p_color.equals(    tempcol =  Color.darkGray ) ) 
    result = "DARKGRAY";
  
  else if ( p_color.equals(     tempcol =  Color.yellow ) ) 
    result = "YELLOW";
  
  else if ( p_color.equals(     tempcol =   Color.green ) ) 
    result = "GREEN";
  
  else if ( p_color.equals(    tempcol =  Color.black )  )
    result = "BLACK";

  else if ( p_color.equals(     tempcol =   Color.cyan ) ) 
    result = "CYAN";

  else if ( p_color.equals(     tempcol =  Color.magenta ) ) 
    result = "MAGENTA";

  else if ( p_color.equals(     tempcol =  Color.orange  ) )
    result = "ORANGE";

  else if ( p_color.equals(     tempcol =   Color.pink  ) )
    result = "PINK";
     ...
     *	deleteFile
     */
    public static boolean deleteFile ( String  filepath , Frame parent )
    {
	UserPref myUserPref = new UserPref();
	...
	if (	tempFile.renameTo(new File( myUserPref.trashDir , new File (filepath).getName()) ) == false ) {
	...
	try { //open file and append
		...
		pout.println((String)linesVector.elementAt(i));
	    ...
	    System.err.println("Error:SCSUtility:stackOnFile:write: IOException: "+filepath);
	    throw new IOException("SCSUtility:stackOnFile:IOException");
	}
    }
    /**---------------------------------------------------------
    ...
    static public     Vector  readLibraryNickAndPathsFile() throws FileNotFoundException, IOException
    {
	boolean done = false ;
	String line = "" ;
	String testdir = "" ;
    ...
    static   String[] vectorToNickAndPathArray ( Vector  nicksAndPaths )
    {
	int i;
	...
	for ( i =0 ; i < nicksAndPaths.size() ; i++ )
	    {
		result_buffer[i]= (String) nicksAndPaths.elementAt(i);
	    }
	...
	while (i<sz) {
	    ...
	    if (line.equals("")) {
		//blank line should not occur but ok
	    } else {
	...
	try { 
	    ...
	    while ( done != true ) { // while nick and paths in file
		line=null;
	...
	try { 
		...
		if (line==null) {
			...
			if (lib_nick==null) {
			    ...
			    return(null);
			} else if (lib_nick.equals(libNickName)==true) {
			    lib_path = parseOutLibPathName(line);
     ...
     * @param String libNickName,  
     * @param String  modulename 
     * @param String version 
     * @throws FileNotFoundException
     * @throws IOException 
     * @returns String that start with the libpath and ends with "/src".
	 */ 
	...
	try {
	    ...
	    if ( tempfile.exists() == true )   // does file exist
		{ 
	...
	if ( pointerToDir.exists() != true ){   // does directory exist

	    System.err.println("ERROR:SCSUtility: file not found: "+pointerToDir.getAbsolutePath());
	    return (null);
	} 
	if ( pointerToDir.isDirectory() != true ){   // is this a directory
	...
	String finalToBeZippedDir=null;
	...
	String canonicalToBeZippedDir=null;
	try {
	    ...
	    folderZipCompressor(zipOutputStream, canonicalToBeZippedDir,toBeZippedDirFile.list());
	...
	for (int i = 0; i < zippedNames.length; i++) {
	    File zippedFile = new File(canonicalToBeZippedDir,zippedNames[i]);
		...
		folderZipCompressor(zipOutputStream, zippedFile.getPath()+File.separator
				    , new File(zippedFile+File.separator).list());
	    }
