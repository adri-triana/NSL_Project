...
public void initText(String location,int pxmin,int pymin,int pxmax,int pymax,String p_text,Font p_font,Color p_color,int p_size)
    ...
    int locationX=pxmin+(int)Math.round((pxmax-pxmin)/2); //center of box
    int locationY=pymin+(int)Math.round((pymax-pymin)/2); //center of box
	...
	return;
      ...
      int wD2=(int)Math.round((fm.stringWidth(p_text)/2));
      int hD2=(int)Math.round((fm.getHeight()/2));      

    // where is the center of the text going
    if (location.equals("CENTER")) {
	//do nothing
    } else if (location.equals("ABOVE")) {
...
//---------------------------------------------------------
  /**
	...
	try {
	  ...
	  //	  os.writeObject(c);
//	  os.writeObject(font);
	//todo: this should take the input from the optinos
...
/**
	...
	//System.out.println("Debug:GraphicPart_text:read text caled..");
	try {

	    super.read(os);
	    //System.out.println("Debug:GraphicPart_text:read super called");
	    //	    c=(Color)os.readObject();
	    //System.out.println("Debug:GraphicPart_text:readread object called ..");
