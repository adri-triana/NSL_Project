...
*/
	...
	*/
	...
	if (actionLabel.equals("Close")) { 
		...
		currModule=(Module)hierarchyStack.pop(); 
	        SchematicPanel.currBackgroundCol=OptionsFrame.drawBack_col;
	        mySchematicPanel.setBackground(SchematicPanel.currBackgroundCol);
		return;
	    } else {
	...
	if (actionLabel.equals("Descend")) { 
	    ...
	    hierarchyStack.push((Object)currModule); 
	...
	if (actionLabel.equals("Free text")) { 
	    ...
	    GetNamePopup ie = new GetNamePopup((Frame)this, SCSUtility.maxCharsFreeText) ; 
	    ie.setSize(300,100); 
		...
		{ 
		    status = "insert_text" ; 
		    mySchematicPanel.freeTextString =  ie.NameTF.getText(); 
		} 
	    else 
		{ 
	...
	if (actionLabel.equals("Icon Editor")) { 
		...
		try {
		    ...
		    // todo: this should bring up the warning box
		    warningPopup.display("Error:SchEditorFrame: IOException for file corresponding to "+selIconInst.moduleName);
	     	    return; 
		}
	    }
	    else {
	...
	*/
	...
	if (actionLabel.equals("BMW Upload")) { 
	    BMWUploadDialog bmwUploadDialog=null;
	    try {
	        bmwUploadDialog= new BMWUploadDialog(this,"BMW Upload");			        bmwUploadDialog.show();
	...
	if ((vari==-1)&&(iconi!=-1)) { // already in icon list
	    //should have already been in variable list
	    boolean worked2=currModule.addVariable(var);
	    return;
	}
	if ((vari!=-1)&&(iconi!=-1)) { // already in both lists
	    warningPopup.display("SchEditorFrame:same name. Cannot add.");
	    return;
	}
    }//end insertIcon
