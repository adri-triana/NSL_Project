 ...
 */
	...
	try {
		...
		for (ix=0;ix<drawableParts.size();ix++) {
			...
			os.writeUTF("Inport");
			((IconInport)part).write(os,  sxmin, symin);
		    }
		    else if (part instanceof IconOutport) {
			os.writeUTF("Outport");
			((IconOutport)part).write(os, sxmin, symin);
		    }
		    else if (part instanceof GraphicPart_line) {
			os.writeUTF("Line");
			((GraphicPart_line)part).write(os, sxmin, symin);
		    }
		    else if (part instanceof GraphicPart_rect) {
			os.writeUTF("Rect");
			((GraphicPart_rect)part).write(os, sxmin, symin);
		    }
		    else if (part instanceof GraphicPart_oval) {
			os.writeUTF("Oval");
			((GraphicPart_oval)part).write(os, sxmin, symin);
		    }
		    else if (part instanceof GraphicPart_poly) {
			os.writeUTF("Poly");
			((GraphicPart_poly)part).write(os, sxmin, symin);
		    }
		    else if (part instanceof GraphicPart_text) {
			//System.out.println("Writing text fields..");
			os.writeUTF("Text");
			((GraphicPart_text)part).write(os,  sxmin, symin);
		    }
		    else {
 ...
 */
	...
	if (drawableParts==null) {
		...
		if (part instanceof IconInport) { 
		    pw.print("InportPin: ");
		    ((IconInport)part).writeAllChars(pw, sxmin, symin);
		    pw.print("\n");
		}
		else if (part instanceof IconOutport) {
		    pw.print("OutportPin: ");
		    ((IconOutport)part).writeAllChars(pw, sxmin, symin);
		    pw.print("\n");
		}
		else if (part instanceof GraphicPart_line) {
		    pw.print("Line: ");
		    ((GraphicPart_line)part).writeAllChars(pw, sxmin, symin);
		    pw.print("\n");
		}
		else if (part instanceof GraphicPart_rect) {
		    pw.print("Rect: ");
		    ((GraphicPart_rect)part).writeAllChars(pw, sxmin, symin);
		    pw.print("\n");
		}
		else if (part instanceof GraphicPart_oval) {
		    pw.print("Oval: ");

		    ((GraphicPart_oval)part).writeAllChars(pw, sxmin, symin);
		    pw.print("\n");
		}
		else if (part instanceof GraphicPart_poly) {
		    pw.print("Poly: ");

		    ((GraphicPart_poly)part).writeAllChars(pw, sxmin, symin);
		    pw.print("\n");
		}
		else if (part instanceof GraphicPart_text) {
		    //System.out.println("Debug:Icon:Writing text fields..");
		    pw.print("Text: ");
		    ((GraphicPart_text)part).writeAllChars(pw, sxmin, symin);
		    pw.print("\n");
		}
		else {
