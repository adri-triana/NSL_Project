...
//--------------------------------------------
	...
	else if (status.equals("move") || status.equals("null") ) {
	...
	if ((var.portIconDirection=='L')&&(x<Icon.pinLength)) {
            String errMsg="Error:IconPanel:addPort:x less than Icon.pinLength"; 	    warningPopup.display(errMsg);
	    return;
	    //top to bottom
	...
	GraphicPart port=(GraphicPart)(new IconInport(var,x,y,c));
	addPort(var,x,y,port);
    } // end addInports

    //--------------------------------------
    public void addOutport(int x, int y) {
	GraphicPart port=(GraphicPart)(new IconOutport(var,x,y,c));
	addPort(var,x,y,port);
    }
    //------------------------------------------------------------
    /**
