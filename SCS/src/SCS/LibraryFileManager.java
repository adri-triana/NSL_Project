...
public class LibraryFileManager extends JFrame implements ListSelectionListener , ActionListener   {
  ...
  Vector nickAndLibPathEntries = new Vector();
  JList libraryList ;
  JList moduleList;
  JList versionList;
  JList srcDocIOList;
...
public LibraryFileManager(  )   {
    ...
    warningDialog=new WarningDialog((Frame)this, "warning");
    warningDialogOkCancel=new WarningDialogOkCancel((Frame)this, "warning");
    ...
    srcDocIOList.setSelectionMode(0);
    //Create the scroll pane and add the table to it. 
 ...
 /* *
  ...
  if (actionLabel.equals("Add Existing Library Path") == true ) {
...
// todo: trying to remove this method
  private  String[] returnDirList( String  parentdirpath )
	...
	if ( tempfile != null )
	  ...
	  // if there is a directory and the album name is not blank  
	  if ( tempfile.isDirectory() == true  &&  ( albumdir.getName().equals(SCSUtility.blankString10) == false ) )
	    {
		{
...
private String returnLibraryPath ( )
    {
      return  ( String ) libraryList.getSelectedValue() ;
...
public void valueChanged(ListSelectionEvent event) 
      ...
      if (event.getSource() instanceof JList ) 
	{
	  ...
	  if ( templist.equals(versionList) == true ){
	    ...
	    fileList = (new File(returnVersionPath())).list() ;
	  ...
	  if ( templist.equals(srcDocIOList) == true ){
	    ...
	    fileList = (new File( returnSrcDocIOPath())).list() ; //throws IO
