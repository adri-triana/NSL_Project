...
class EditorFrame extends JFrame{   //JFrames betterthan Frame
	...
	if ((this.currModule!=null)&&(this.savedInEd==false)&&(noActionTaken==false))  { 
	...
	try {
	    ...
	    tfs= new TableFileSelector( this , "Module",wantFile);
	}
	...
	try { 
	    ...
	    if (newSrcDir.isDirectory()==false) {
		newSrcDir.mkdirs();
	    }
	...
	if (this.currModule==null) { 
	    ...
	    endCloseTool();
	    return(worked=true); //todo: verify you do not need finally stuff
	}
	if (noActionTaken==true) {
	...
	boolean worked2=executiveFrame.windowsAndModulesOpen.remove(sm); 
	if (!worked2) {
	    String errstr2= "EditorFrame: could not remove module from vector"; 	    warningPopup.display(errstr2);
	...
	if (sz<=0) { //windows may be open, but nothing in them
	    ...
	    String winstr="";//list first 5 window open
	    if (sz>=1) {
		StringModule sm1=(StringModule)executiveFrame.windowsAndModulesOpen.elementAt(0);
		Module m1=sm1.module1;
		if (sm1.module1!=null) {
		winstr=m1.moduleName;
		}
	    }
	    for (int i=1; ((i<sz)&&(i<5)); i++) {
		StringModule sm1=(StringModule)executiveFrame.windowsAndModulesOpen.elementAt(i);
		Module m1=sm1.module1;
		if (sm1.module1!=null) {
		    winstr=winstr+","+m1.moduleName;
		}
	    }
